...
final class ProvidersController {
    ...
    public function index(): void {
        ...
        try {
            ...
            $clauses = [];
            $params = [];
            if ($status !== 'all') { $clauses[] = '(LOWER(status) = LOWER(?) OR (verified = 1 AND ? = ' . $db->quote('verified') . '))'; $params[] = $status; }
            if ($search !== '') { $clauses[] = '(LOWER(name) LIKE ? OR LOWER(email) LIKE ?)'; $params[] = '%' . strtolower($search) . '%'; $params[] = '%' . strtolower($search) . '%'; }
            $where = $clauses ? ('WHERE ' . implode(' AND ', $clauses)) : '';
            $stmtCount = $db->prepare("SELECT COUNT(*) FROM providers {$where}");
            $stmtCount->execute($params);
            $total = (int)($stmtCount->fetchColumn() ?: 0);
            $sql = "SELECT id, name, email, verified, status, rating, subscription_status, subscription_expires_at FROM providers {$where} ORDER BY id DESC LIMIT :limit OFFSET :offset";
            $stmt = $db->prepare($sql);
