...
type Item = { title: string; price?: string; size?: string; desc?: string; section?: string };
    ...
    <View style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={styles.container}>
        {pageTitle ? (
          <View style={styles.pageHeader}>
            ...
            {pageSubtitle ? <View style={styles.pageUnderline} /> : null}
          </View>
        ) : null}
        {selected.length > 0 ? (
          ...
          return (
            <TouchableOpacity key={idx} style={[styles.categoryBox, isSelected && styles.selectedBox]} onPress={() => toggle(service.title)}>
              <View style={styles.categoryContent}>
                ...
                {service.size ? <Text style={styles.categorySize}>{service.size}</Text> : null}
      ...
      <View style={styles.footer}>
        <View style={styles.totalBox}>
          ...
          <Text style={styles.totalValue}>{formatTotal(selected, items)}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  pageTitle: { fontSize: 24, fontWeight: 'bold', color: '#000', marginBottom: 4 },
  pageSubtitle: { fontSize: 18, fontWeight: '700', color: '#00ADB5' },
  pageUnderline: { height: 3, backgroundColor: '#00ADB5', width: 140, marginTop: 6, borderRadius: 2 },
  categoryBox: {
  ...
  categorySize: { fontSize: 14, color: '#444', marginBottom: 4, fontWeight: '700' },
  ...
  totalBox: { position: 'absolute', right: 16, top: 16, flexDirection: 'row', alignItems: 'center' },
  totalLabel: { fontSize: 14, color: '#222', marginRight: 6 },
  totalValue: { fontSize: 14, fontWeight: '700', color: '#222' },
...
function formatTotal(selected: string[], items: Item[]) {
  const map = new Map<string, Item>();
  items.forEach((it) => map.set(it.title, it));
  let sum = 0;
  selected.forEach((t) => {
    const it = map.get(t);
    if (!it || !it.price) return;
    const m = it.price.match(/₱\s*([\d,]+\.?\d*)/);
    if (m) {
